...
class Slice {
   ...
   *  @return   string
   */
  protected function _compile_error($content)
  {
    $pattern = '/(\s*)@error\s*\((.*?)\)/';

    return preg_replace($pattern, '$1<?php if (isset($errors) && isset($errors[$2])): ?>', $content);
  }
  ...
  {
    $pattern = '/(\s*)@(endunless|endisset|endempty|enderror)/';
